...
**Amendments 2026-09-04 (final reviews).** Gateway: the held claim sweeps stale leases once per request, not per tick; the cron sweep covers the rest, so requeue latency after a lost lease is up to `wait` (25 s) longer. `ClaimResponse.job` carries `chainId`; the agent validates it against its configured `CHAIN_ID` when present and tolerates its absence. Lease writes by node state: item 9 in section 7. Sandbox chain and contract addresses come from agent env (`CHAIN_ID`, `DATA_REGISTRY_CONTRACT`, …) with Moksha defaults; `STORAGE_API_URL` defaults by chain; sandboxes run with `--memory/--cpus/--pids-limit`; the agent reconciles orphaned `ps-*` containers at boot and drains on SIGTERM within `stop_grace_period`.
...
| `POST /v1/jobs/claim?wait=0..25`                        | node (state `admitted`)      | `ClaimRequest` → 200 `ClaimResponse` \| 204                             | sweep stale leases once at entry (`claim.ts`), then claim every `CLAIM_POLL_FLOOR_MS` until `wait` expires; `maxDuration: 30`; 403 `NODE_NOT_ADMITTED` (draining nodes get 204)                             |
...
8. **Public API changes in personal-server-ts #245** approved under the visibility rule: `redactEnvelopeForGrantee` exported, sync adapter `reads` option, `createServer` `profile`/`serverAccount`, `AppDeps.profile`/`AppDeps.jobWorker`, `createSyncManager` `transferMode`. Approved 2026-09-04 in addition: `createPublicOnlyAccount` (core/keys), `verifySignedArtifacts` and `deriveBuilderId` (core/policy), and the three enclave error classes in core/errors.
9. **Lease writes by node state (data-gateway #108, confirmed 2026-09-04).** `draining` stays graceful: no new claims, heartbeat/complete/fail still accepted so in-flight jobs finish. `pending`/`removed` nodes get 409 `LEASE_LOST` on heartbeat/complete/fail; the agent aborts the sandbox and the sweep requeues the job.
10. **Crash recovery is proven with a crash lever, not drain (2026-09-04).** Drain does not abort in-flight leases (item 9), so the recovery test stops the slow node's CVM (`phala cvms stop`) while a job is claimed: the lease expires (`LEASE_SECONDS=30`), the sweep requeues, the second node completes attempt 2. PASSED 2026-09-04 on fleet spike-b6 (head 9503461, gateway ca45c27): the slow node's CVM was stopped mid-job; the replica completed attempt 2 in 48.5 s. The same run proved `scripts/tee/update.sh` live: one CVM rolled in place to a new commit and compose hash, re-registered under a new node id, old id drained then removed (remove without drain is refused).
11. **DCR status carries `delivery: "enclave"` through the SDK (2026-09-04).** The direct access-request client's status mapping keeps `delivery` so consumers can branch on it (Lorebook direct mode showed `not_ready` instead of the enclave path). Lands in vana-sdk #207 before its release.
12. **SDK pins at merge time (2026-09-04).** vana-sdk #207 merges first and semantic-release publishes 3.22.x; personal-server-ts, data-gateway, web and Lorebook re-pin to that release in one chore commit each before they merge. Account stays on `pr-208` (dev already pins a #186 prerelease) and moves when #186 lands. Merge order: vana-sdk #207, personal-server-ts #245 then #246, data-gateway #100, unity-surfaces #987 (dev), lorebook #1.
