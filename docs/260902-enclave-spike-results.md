...
Date: 2026-09-02 (`date -u +%F`). Operator: Codex on Phala profile `volod-vana` (`phala status`). Branch `spike/sandbox`, final commit `74b3019`, not pushed (`git rev-parse --abbrev-ref HEAD; git rev-parse --short HEAD`). CVMs: `spike-sandbox-sysbox`, `-privileged`, `-plain`, `-agent`, `-gvisor`; dstack OS `0.5.9`, image hash prefix `bd369a8c`, measurement node `prod5` / id 26, `tdx.medium` = 2 vCPU / 4 GB (`phala cvms get 48fb55d1-c42e-4345-8f89-d2c34ba65839 --json`).

Sources: dstack runtime facts in Spike 0 Q5; gVisor installation/platform/Docker/production docs at https://gvisor.dev/docs/user_guide/install/, https://gvisor.dev/docs/user_guide/platforms/, https://gvisor.dev/docs/user_guide/quick_start/docker/, and https://gvisor.dev/docs/user_guide/production/ (accessed 2026-09-02); installed `runsc release-20260817.0` (`phala logs sandbox-agent --cvm-id 48fb55d1-c42e-4345-8f89-d2c34ba65839 --stderr -n 5600 | rg 'runsc version|spec:'`).

### Isolation result

Strongest working per-user boundary: gVisor `runsc` ptrace platform, installed in a socket-free trusted privileged DinD management container. The Personal Server ran as uid/gid 1000, all five capability masks were zero, root was read-only 9p, `/data` was a 256 MiB `rw,noexec,nosuid` tmpfs, `/sys` and cgroup were read-only, and neither `/var/run/dstack.sock` nor `/var/run/docker.sock` existed (`phala logs sandbox-agent --cvm-id 48fb55d1-c42e-4345-8f89-d2c34ba65839 --stderr -n 5600`, section `sandbox socket/capability evidence`). Only the trusted agent saw dstack + host Docker sockets; the trusted runtime saw its nested Docker socket but no dstack socket (same command, sections `trusted agent` and `trusted runtime socket evidence`).

Phala accepted and started all three probes: Sysbox, privileged, and plain hardened (`phala ps --cvm-id abae41db-1b14-4eb1-a9d0-9cddf1044956 --json`; same with `2b875ebf-cd23-4c18-82c8-837f0b23fa0e` and `f37dbc49-90c0-4d17-94c9-f4c736459784`). Sysbox 0.6.7 ran nested Docker with uid 0 mapped to host uid 100000 and a nested Alpine workload with zero capabilities/no host sockets (`phala logs sysbox-probe --cvm-id abae41db-1b14-4eb1-a9d0-9cddf1044956 --stderr -n 240`; `phala logs sysbox-probe --cvm-id abae41db-1b14-4eb1-a9d0-9cddf1044956 --stderr -n 240 | rg '0.6.7|100000|Cap'`). Plain runc ran uid 1000, zero capabilities, read-only root, and no sockets (`phala logs plain-probe --cvm-id f37dbc49-90c0-4d17-94c9-f4c736459784 --stderr -n 120`). gVisor systrap and ptrace both failed when nested inside Sysbox with `cannot read client sync file ... EOF`; direct gVisor initially needed the trusted DinD's host cgroup namespace (`phala logs sandbox-agent --cvm-id 48fb55d1-c42e-4345-8f89-d2c34ba65839 --stderr -n 1200`).

The published image `vanaorg/personal-server@sha256:dc6dfd47cd658aa05d473863d6589f8e2d086c2c5cc7368ef0f4e8a212eb6bb9` was pullable but exited with `ERR_MODULE_NOT_FOUND: @opendatalabs/vana-sdk` (same log command). Measurements used a derived disposable image that added declared SDK `3.14.0`; image build/pull/setup time is excluded.

### Numbers

| metric                                               |                   p50 |          p95 |    runs | command                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------------------------------- | --------------------: | -----------: | ------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| warm-image agent start -> `/health` 200              |          7,461.393 ms | 7,704.755 ms |      10 | raw `readyMs`: `phala logs sandbox-agent --cvm-id 48fb55d1-c42e-4345-8f89-d2c34ba65839 --stderr -n 5600`; calculation: `node -e 'const x=[7704.754939,7424.196123,7635.732446,7156.658885,7450.620135,7679.353195,7225.891318,7472.435783,7365.519683,7472.164938].sort((a,b)=>a-b); console.log({p50:(x[4]+x[5])/2,p95:x[Math.ceil(.95*x.length)-1]})'` |
| idle sandbox memory                                  |             119.2 MiB |          n/a |       1 | `phala logs sandbox-agent --cvm-id 48fb55d1-c42e-4345-8f89-d2c34ba65839 --stderr -n 5600`, section `idle stats`, inner `docker stats --no-stream ps-run-1`                                                                                                                                                                                               |
| active sandbox memory (tight `/health` loop for 5 s) |             136.2 MiB |          n/a |       1 | same authoritative log command, section `active stats`; load command is embedded in `deploy/dstack/docker-compose.sandbox.yml`                                                                                                                                                                                                                           |
| trusted runtime total, idle / active                 | 182.7 MiB / 183.5 MiB |          n/a |  1 each | same authoritative log command, outer `docker stats --no-stream sandbox-runtime`                                                                                                                                                                                                                                                                         |
| `tdx.2xlarge` memory-only upper bound                |         240 sandboxes |          n/a | derived | capacity `32,768 MiB` from `phala instance-types --json`; `node -e 'const hostMiB=32768,activeSandboxMiB=136.2,sharedRuntimeMiB=183.5-activeSandboxMiB; console.log(Math.floor((hostMiB-sharedRuntimeMiB)/activeSandboxMiB))'`                                                                                                                           |
