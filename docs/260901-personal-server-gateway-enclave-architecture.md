...
**Result delivery (revised 2026-09-04).** Every result goes to object storage; there is no inline ciphertext path. The runtime stream-encrypts to the builder key and PUTs to the `jobresults/` prefix of the `vana-storage` bucket, then completes the job with metadata only (key, hash, size, expiry). The builder reads the object directly. The reasons are: Vercel caps a function request **or** response body at 4.5 MB, so an inline column can never carry a 20-50 MB scope and the pre-enclave direct read had no ceiling at all; a single path removes the size cliff, the tuning constant, and one of two expiry mechanisms; and keeping ciphertext off the Gateway leaves it a pure control plane. Objects are ECIES-sealed to the builder, so reads are public like every other blob (see `vana-storage` below) and the R2 lifecycle rule on the prefix expires them without anyone issuing deletes. The job row keeps the logical expiry the Gateway enforces when it stops serving a handle; the lifecycle rule is the physical backstop. There is no separate `result_handles` table: the relation is 1:1 with a job and `bucket` is config, not row data.

**Result upload signing (added 2026-09-04).** The sandbox cannot sign the storage PUT and must not be able to. It receives only `PS_SERVER_ADDRESS` and `PS_SERVER_PUBLIC_KEY`; `enclave-main.ts` builds a public-only account whose `signMessage()` throws, and the dstack-derived server key never leaves the node agent. Reads never needed a signature because blob `GET` is public, so writing results is new capability. The agent therefore exposes one narrow route, `POST /agent/v1/job-results/sign`: the sandbox sends the job id, chain id, byte length and the `sha256` of the sealed bytes, never the bytes, and receives an `Authorization` header for exactly one upload. It is a constrained signing oracle, not a general one. The agent builds the host, method and URI itself and accepts none of them from the caller, so it cannot be asked to sign a different request; it authenticates the per-sandbox bearer token rather than the global operator secret; it requires the job to be bound to that sandbox and still executing, with the binding removed in a `finally`; it recomputes `userPsId` and re-derives the server wallet and requires both to match the sandbox binding; and it binds the caller's body hash into the signature so one cannot be replayed against different bytes. This is new agent surface inside every user's TCB and is reviewed as such. `RESULT_TOO_LARGE` survives with new meaning: storage refuses above 100 MB, and that refusal is not retryable.
...
4. PS verifies grant, registration, and builder key from signed artifacts; checks revocation on chain; redacts grantee-hidden fields; encrypts to the builder key; asks the node agent to sign the upload (the sandbox holds no signing key, see below); PUTs the sealed bytes into `jobresults/`, then completes the job with the handle. Receipt issues on the handle commit.
...
packages/enclave     + node agent: heartbeat, claim, sandbox lifecycle, dstack key agent, envelope sealing,
                       job-result upload signing (see Result upload signing)
...
db                   + tee_nodes, jobs (result_object_key/hash/size/expiry inline on the row),
                       job_attempts, identity_records, sealed_secrets,
                       question_intents
