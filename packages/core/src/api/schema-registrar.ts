...
import type { RegisteredSchema, SchemaRegistrarPort } from "../ports/index.js";
...
export interface SchemaRegistrarDeps {
  gatewayUrl: string;
  signer: Pick<ServerSigner, "address" | "signSchemaRegistration">;
 ...
 *
 * Lives in core (not the Node server) so the in-browser PS-Lite runtime can
 * register no-schema schemas for binary uploads too — otherwise the sync
 * upload worker fails with "No schema found for scope".
 */
