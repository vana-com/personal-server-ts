...
- Stable release: `release.yml` runs semantic-release on push to `main` and publishes `packages/core` → `server` → `cli` (see `.releaserc.yaml`). Workspace `*` deps are pinned to the release version before publish via `scripts/semantic-release-pin-workspace-deps.cjs` (same idea as canary).
- Canary prereleases: `prerelease.yml` on `dev`/`develop`/`feat/*` rewrites package versions/pins then `npm publish --tag canary` for core/server/cli.
