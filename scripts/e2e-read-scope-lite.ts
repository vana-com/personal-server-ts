...
async function main() {
  ...
  // JSON scope: read_scope returns readable blocks.
  const jsonRead = toolJson(
    (
      await mcpRpc(runtime, token, "tools/call", {
        name: "read_scope",
        arguments: { scope: JSON_SCOPE, maxBytes: 65536 },
      })
    ).body,
  );
  check(
    Array.isArray(jsonRead?.blocks) && !jsonRead?.error,
    `${JSON_SCOPE}: JSON blocks readable`,
    jsonRead?.error ?? `contentKind=${jsonRead?.contentKind}`,
  );

  // Binary scope: read_scope surfaces metadata blocks; the raw bytes come from
  // resources/read (vana://scope/<scope>/raw), which is how the browser runtime
  // serves binary content over MCP.
  ...
  check(
    ...
    `${BINARY_SCOPE}: read_scope metadata blocks`,
    binMeta?.error ?? `contentKind=${binMeta?.contentKind}`,
  );

  const rawRead = await mcpRpc(runtime, token, "resources/read", {
    uri: `vana://scope/${BINARY_SCOPE}/raw`,
  });
  const content = rawRead.body?.result?.contents?.[0] as
    | { blob?: string; mimeType?: string; _meta?: { sizeBytes?: number } }
    | undefined;
  check(
    rawRead.ok && typeof content?.blob === "string",
    `${BINARY_SCOPE}: resources/read returns raw binary`,
  ...
  check(
    sha256hex(decoded) === wantHash,
    `${BINARY_SCOPE}: bytes match original PDF (sha256)`,
    sha256hex(decoded) === wantHash ? "" : `len ${decoded.length}`,
  );
